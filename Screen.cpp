...
namespace particles
{
		...
		m_texture(NULL)
	{
		...
		SCREEN_HALF_WIDTH = W / 2;
		SCREEN_HALF_HEIGHT = SCREEN_HEIGHT / 2;

		m_buffer1 = new Uint32[W * SCREEN_HEIGHT];
		m_buffer2 = new Uint32[W * SCREEN_HEIGHT];
		memset(m_buffer1, 0, W * SCREEN_HEIGHT * sizeof(Uint32));
		memset(m_buffer2, 0, W * SCREEN_HEIGHT * sizeof(Uint32));
		minBlurTime = INT_MAX;
		maxBlurTime = 0;
	...
	void Screen::boxBlur()
	{
		...
		for (int x=0; x<(W-2); x++)
		{
			...
			red +=
				(Uint8)(m_buffer2[x+W] >> 24) +
				(Uint8)(m_buffer2[x+W + 1] >> 24) +
				(Uint8)(m_buffer2[x+W + 2] >> 24);	// 7 8 9
			green +=
				(Uint8)(m_buffer2[x+W] >> 16) +
				(Uint8)(m_buffer2[x+W + 1] >> 16) + (Uint8)
				(m_buffer2[x+W + 2] >> 16);
			blue += 
				(Uint8)(m_buffer2[x+W] >> 8) +
				(Uint8)(m_buffer2[x+W + 1] >> 8) +
				(Uint8)(m_buffer2[x+W + 2] >> 8);
		...
		//
		r = W * (SCREEN_HEIGHT - 2);	// start of the row before last one
		z = r + 1;
		t = r + 2;
		for (Uint32 x = 0; x < (Uint32)(W - 2); x++)
		{
			// 1 2 3
		...
		for ( int n=0; n<(SCREEN_HEIGHT-2); n++)
		{
			...
			y = y - W;
			setPixel((W-1), (n + 1), (Uint8)(red / 9), (Uint8)(green / 9), (Uint8)(blue / 9));
		}	// end for y
		...
		l = W - 2;	// top right pixel at the box corner
		red =   (Uint8)(m_buffer2[l] >> 24) + (Uint8)(m_buffer2[l+1] >> 24);
		green = (Uint8)(m_buffer2[l] >> 16) + (Uint8)(m_buffer2[l+1] >> 16);
		blue =  (Uint8)(m_buffer2[l] >>  8) + (Uint8)(m_buffer2[l+1] >>  8);
		l = l + W;
		...
		green += (Uint8)(m_buffer2[l] >> 16) + (Uint8)(m_buffer2[l+1] >> 16);
		blue +=  (Uint8)(m_buffer2[l] >>  8) + (Uint8)(m_buffer2[l+1] >>  8);
		setPixel((W - 1), 0, (Uint8)(red / 9), (Uint8)(green / 9), (Uint8)(blue / 9));
		// Bottom Left (0,(SCREEN_HEIGHT-2))
		l = (SCREEN_HEIGHT - 2) * W;	// top left pixel at the corner
		red =   (Uint8)(m_buffer2[l] >> 24) + (Uint8)(m_buffer2[l+1] >> 24);
		green = (Uint8)(m_buffer2[l] >> 16) + (Uint8)(m_buffer2[l+1] >> 16);
		blue =  (Uint8)(m_buffer2[l] >>  8) + (Uint8)(m_buffer2[l+1] >>  8);
		l = W;
		...
		l = (SCREEN_HEIGHT - 2) * W + W - 2;	// top left pixel at the corner
		red =   (Uint8)(m_buffer2[l] >> 24) + (Uint8)(m_buffer2[l+1] >> 24);
		green = (Uint8)(m_buffer2[l] >> 16) + (Uint8)(m_buffer2[l+1] >> 16);
		blue =  (Uint8)(m_buffer2[l] >>  8) + (Uint8)(m_buffer2[l+1] >>  8);
		l = W;
		...
		green += (Uint8)(m_buffer2[l] >> 16) + (Uint8)(m_buffer2[l+1] >> 16);
		blue +=  (Uint8)(m_buffer2[l] >> 8) +  (Uint8)(m_buffer2[l+1] >> 8);
		setPixel((W - 1), (SCREEN_HEIGHT - 1), (Uint8)(red / 9), (Uint8)(green / 9), (Uint8)(blue / 9));
		// stop timer
			...
			for (int x = 0; x < W; x++) {
				...
				for (int row = -1; row <= 1; row++) {
					for (int col = -1; col <= 1; col++) {
						...
						if (currentX >= 0 && currentX < W && currentY >= 0 && currentY < SCREEN_HEIGHT) {
							Uint32 color = m_buffer2[currentY * W + currentX];
	...
	bool Screen::init()
	{
		...
		SDL_UpdateTexture(m_texture, NULL, m_buffer1, W * sizeof(Uint32));
